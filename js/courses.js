...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saveButton = document.getElementById("saveNewQuestionBtn");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (editingCourseId) {
      await updateCourse(editingCourseId, { CourseName: courseName });
    } else {
      await handleCreateCourse(courseName);
    }
  ...
  document.body.addEventListener("click", async (e) => {
    if (e.target.classList.contains("edit-course-btn")) {
      ...
      if (courseDoc.exists()) {
        ...
        courseInput.value = courseData.CourseName;
...
async function loadCourses() {
  try {
    const courseContainer = document.getElementById("rowcourse");
    courseContainer.innerHTML = ""; // Clear previous content completely
    let courseModelList = [];
    ...
    // Load courses based on the course count
    ...
    // Reattach event listeners after loading courses
